...
public class MyHrEmployeeServiceImpl implements MyHrEmployeeService {
    ...
    @Override
    public ResponseEntity<?> downloadEmployeeListAsExcel() {

        logger.info("Generating employee Excel file");

        try {
            List<Object[]> employees = listEmployees();

            String fileName = excelPrefix + System.currentTimeMillis() + ".xlsx";
            ...
            for (Object[] emp : employees) {
                ...
                row.createCell(0).setCellValue(String.valueOf(emp[0]));
                row.createCell(1).setCellValue(String.valueOf(emp[1]));
            }
            ...
            logger.info("Employee Excel generated successfully: {}", fileName);
            ...
            logger.error("Failed to generate employee Excel file", e);

            Map<String, Object> error = new HashMap<>();
            error.put("status", "error");
            error.put("errorCode", "EXCEL_GENERATION_FAILED");
